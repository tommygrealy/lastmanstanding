...
--
-- Host: localhost:3306
-- Generation Time: Aug 24, 2022 at 01:56 PM
...
--
CREATE DATABASE IF NOT EXISTS `lastmanstanding` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;
USE `lastmanstanding`;
...
-- Procedures
--
DROP PROCEDURE IF EXISTS `cancelPrediction`$$
CREATE PROCEDURE `cancelPrediction` (IN `inPredictionID` INT(11), IN `inUserName` VARCHAR(255))  BEGIN
...
DROP PROCEDURE IF EXISTS `checkResultsVsPredictions`$$
CREATE PROCEDURE `checkResultsVsPredictions` ()  BEGIN
...
DROP PROCEDURE IF EXISTS `generateResetForUsername`$$
CREATE PROCEDURE `generateResetForUsername` (IN `inUsername` VARCHAR(255), IN `inToken` VARCHAR(45))  NO SQL
...
DROP PROCEDURE IF EXISTS `getNextFixtureForTeam`$$
CREATE PROCEDURE `getNextFixtureForTeam` (IN `TeamNameLong` VARCHAR(50))  BEGIN
...
DROP PROCEDURE IF EXISTS `getUserDetailsFromToken`$$
CREATE PROCEDURE `getUserDetailsFromToken` (IN `inToken` VARCHAR(45))  NO SQL
...
CREATE PROCEDURE `insertPrediction` (IN `inFixtureID` INT(11), IN `inUserName` VARCHAR(255), IN `inPredictedResult` INT(11), IN `inEntryType` VARCHAR(10))  BEGIN
...
DROP PROCEDURE IF EXISTS `passwordReset`$$
CREATE PROCEDURE `passwordReset` (IN `inUsername` VARCHAR(255), IN `inPassword` VARCHAR(64), IN `inSalt` VARCHAR(16))  NO SQL
update users 
set password=inPassword, salt=inSalt
where username = inUsername$$

DROP PROCEDURE IF EXISTS `selectRandomTeam`$$
CREATE PROCEDURE `selectRandomTeam` (IN `inUser` VARCHAR(255))  NO SQL
...
CREATE PROCEDURE `showAvailableTeamsForUser` (IN `inUserName` VARCHAR(255))  READS SQL DATA
BEGIN
...
DROP PROCEDURE IF EXISTS `showCurrentSelections`$$
CREATE PROCEDURE `showCurrentSelections` ()  NO SQL
...
CREATE PROCEDURE `showCurrentStandings` ()  BEGIN
select username, FullName, lives, CompStatus from users where PaymentStatus='Paid';

END$$

DROP PROCEDURE IF EXISTS `showCurrentStandingsByLeague`$$
CREATE PROCEDURE `showCurrentStandingsByLeague` (IN `league_id` INT(11))  BEGIN
select username, FullName, lives, CompStatus from users where PaymentStatus='Paid' 
AND username IN (SELECT username from users_join_leagues WHERE users_join_leagues.league_id = league_id);

END$$

DROP PROCEDURE IF EXISTS `showNullResultFixtures`$$
CREATE PROCEDURE `showNullResultFixtures` ()  NO SQL
...
DROP PROCEDURE IF EXISTS `showPredsByGameWeek`$$
CREATE PROCEDURE `showPredsByGameWeek` (IN `inGameWeek` INT)  NO SQL
select * from predictions where gameweek=inGameWeek$$

DROP PROCEDURE IF EXISTS `showSelectionsPostDeadline`$$
CREATE PROCEDURE `showSelectionsPostDeadline` ()  NO SQL
    COMMENT 'Shows all user selections for the next round of games.'
BEGIN
...
CREATE PROCEDURE `showUserCurrentSelection` (IN `inUserName` VARCHAR(255))  BEGIN
...
CREATE PROCEDURE `showUserPredictionHistory` (IN `inUsername` VARCHAR(255))  BEGIN
...
DROP PROCEDURE IF EXISTS `updateMatchScore`$$
CREATE PROCEDURE `updateMatchScore` (IN `inFixtureID` INT(11), IN `inHomeScore` INT(11), IN `inAwayScore` INT(11), IN `inResult` INT(11))  BEGIN
...
CREATE PROCEDURE `updatePaymentStatus` (IN `inUserName` VARCHAR(255), IN `inPayStat` VARCHAR(45))  BEGIN
...
CREATE TABLE `clubs` (
  ...
  `LongName` varchar(100) CHARACTER SET utf8 NOT NULL,
  `MedName` varchar(100) CHARACTER SET utf8 NOT NULL,
  `ShortName` varchar(8) CHARACTER SET utf8 NOT NULL,
  `CrestURLSmall` varchar(255) CHARACTER SET utf8 NOT NULL,
  `CresURLLarge` varchar(255) CHARACTER SET utf8 NOT NULL
...
(79, 'Brentford', 'Brentford', 'BRE', 'https://ssl.gstatic.com/onebox/media/sports/logos/QOUce0WQBYqnkSmN6_TxGA_48x48.png', 'https://ssl.gstatic.com/onebox/media/sports/logos/QOUce0WQBYqnkSmN6_TxGA_96x96.png'),
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `detailedpredictions`;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Result (1=home win, 2=Draw, 3=away win)';
...
(1335, '2022-08-05 19:00:00', 'Crystal Palace', 'Arsenal', 0, 2, 3),
(1336, '2022-08-06 11:30:00', 'Fulham', 'Liverpool', 2, 2, 2),
(1337, '2022-08-06 13:00:00', 'Leicester', 'Brentford', 2, 2, 2),
(1338, '2022-08-06 14:00:00', 'Bournemouth', 'Aston Villa', 2, 0, 1),
(1339, '2022-08-06 14:00:00', 'Leeds', 'Wolves', 2, 1, 1),
(1340, '2022-08-06 14:00:00', 'Newcastle', 'Nottingham Forest', 2, 0, 1),
(1341, '2022-08-06 14:00:00', 'Spurs', 'Southampton', 4, 1, 1),
(1342, '2022-08-06 16:30:00', 'Everton', 'Chelsea', 0, 1, 3),
(1343, '2022-08-07 13:00:00', 'Man Utd', 'Brighton', 1, 2, 3),
(1344, '2022-08-07 15:30:00', 'West Ham', 'Man City', 0, 2, 3),
(1345, '2022-08-13 12:30:00', 'Aston Villa', 'Everton', 4, 0, 1),
(1346, '2022-08-13 15:00:00', 'Arsenal', 'Leicester', 4, 2, 1),
(1347, '2022-08-13 15:00:00', 'Brighton', 'Newcastle', 0, 0, 2),
(1348, '2022-08-13 15:00:00', 'Man City', 'Bournemouth', 4, 0, 1),
(1349, '2022-08-13 15:00:00', 'Southampton', 'Leeds', 2, 2, 2),
(1350, '2022-08-13 15:00:00', 'Wolves', 'Fulham', 0, 0, 2),
(1351, '2022-08-13 17:30:00', 'Brentford', 'Man Utd', 4, 0, 1),
(1352, '2022-08-14 14:00:00', 'Nottingham Forest', 'West Ham', 1, 0, 1),
(1353, '2022-08-14 16:30:00', 'Chelsea', 'Spurs', 2, 2, 2),
(1354, '2022-08-15 20:00:00', 'Liverpool', 'Crystal Palace', 1, 1, 2),
(1355, '2022-08-20 12:30:00', 'Spurs', 'Wolves', 1, 0, 1),
(1356, '2022-08-20 14:00:00', 'Crystal Palace', 'Aston Villa', 3, 1, 1),
(1357, '2022-08-20 14:00:00', 'Everton', 'Nottingham Forest', 1, 1, 2),
(1358, '2022-08-20 14:00:00', 'Fulham', 'Brentford', 3, 2, 1),
(1359, '2022-08-20 14:00:00', 'Leicester', 'Southampton', 1, 2, 3),
(1360, '2022-08-20 16:30:00', 'Bournemouth', 'Arsenal', 0, 3, 3),
(1361, '2022-08-21 13:00:00', 'Leeds', 'Chelsea', 3, 0, 1),
(1362, '2022-08-21 13:00:00', 'West Ham', 'Brighton', 0, 2, 3),
(1363, '2022-08-21 15:30:00', 'Newcastle', 'Man City', 3, 3, 2),
(1364, '2022-08-22 19:00:00', 'Man Utd', 'Liverpool', 2, 1, 1),
(1365, '2022-08-27 12:30:00', 'Southampton', 'Man Utd', NULL, NULL, NULL),
(1366, '2022-08-27 15:00:00', 'Brentford', 'Everton', NULL, NULL, NULL),
(1367, '2022-08-27 15:00:00', 'Brighton', 'Leeds', NULL, NULL, NULL),
(1368, '2022-08-27 15:00:00', 'Chelsea', 'Leicester', NULL, NULL, NULL),
(1369, '2022-08-27 15:00:00', 'Liverpool', 'Bournemouth', NULL, NULL, NULL),
(1370, '2022-08-27 15:00:00', 'Man City', 'Crystal Palace', NULL, NULL, NULL),
(1371, '2022-08-27 17:30:00', 'Arsenal', 'Fulham', NULL, NULL, NULL),
(1372, '2022-08-28 14:00:00', 'Aston Villa', 'West Ham', NULL, NULL, NULL),
(1373, '2022-08-28 14:00:00', 'Wolves', 'Newcastle', NULL, NULL, NULL),
(1374, '2022-08-28 16:30:00', 'Nottingham Forest', 'Spurs', NULL, NULL, NULL),
(1375, '2022-08-30 19:30:00', 'Fulham', 'Brighton', NULL, NULL, NULL),
(1376, '2022-08-30 19:30:00', 'Crystal Palace', 'Brentford', NULL, NULL, NULL),
(1377, '2022-08-30 19:45:00', 'Southampton', 'Chelsea', NULL, NULL, NULL),
(1378, '2022-08-30 20:00:00', 'Leeds', 'Everton', NULL, NULL, NULL),
(1379, '2022-08-31 19:30:00', 'Bournemouth', 'Wolves', NULL, NULL, NULL),
(1380, '2022-08-31 19:30:00', 'Arsenal', 'Aston Villa', NULL, NULL, NULL),
(1381, '2022-08-31 19:30:00', 'Man City', 'Nottingham Forest', NULL, NULL, NULL),
(1382, '2022-08-31 19:45:00', 'West Ham', 'Spurs', NULL, NULL, NULL),
(1383, '2022-08-31 20:00:00', 'Liverpool', 'Newcastle', NULL, NULL, NULL),
(1384, '2022-09-01 20:00:00', 'Leicester', 'Man Utd', NULL, NULL, NULL),
(1385, '2022-09-03 12:30:00', 'Everton', 'Liverpool', NULL, NULL, NULL),
(1386, '2022-09-03 15:00:00', 'Brentford', 'Leeds', NULL, NULL, NULL),
(1387, '2022-09-03 15:00:00', 'Newcastle', 'Crystal Palace', NULL, NULL, NULL),
(1388, '2022-09-03 15:00:00', 'Nottingham Forest', 'Bournemouth', NULL, NULL, NULL),
(1389, '2022-09-03 15:00:00', 'Spurs', 'Fulham', NULL, NULL, NULL),
(1390, '2022-09-03 15:00:00', 'Wolves', 'Southampton', NULL, NULL, NULL),
(1391, '2022-09-03 17:30:00', 'Aston Villa', 'Man City', NULL, NULL, NULL),
(1392, '2022-09-04 14:00:00', 'Brighton', 'Leicester', NULL, NULL, NULL),
(1393, '2022-09-04 14:00:00', 'Chelsea', 'West Ham', NULL, NULL, NULL),
(1394, '2022-09-04 16:30:00', 'Man Utd', 'Arsenal', NULL, NULL, NULL),
(1395, '2022-09-10 12:30:00', 'Fulham', 'Chelsea', NULL, NULL, NULL),
(1396, '2022-09-10 15:00:00', 'Bournemouth', 'Brighton', NULL, NULL, NULL),
(1397, '2022-09-10 15:00:00', 'Leicester', 'Aston Villa', NULL, NULL, NULL),
(1398, '2022-09-10 15:00:00', 'Liverpool', 'Wolves', NULL, NULL, NULL),
(1399, '2022-09-10 15:00:00', 'Southampton', 'Brentford', NULL, NULL, NULL),
(1400, '2022-09-10 17:30:00', 'Man City', 'Spurs', NULL, NULL, NULL),
(1401, '2022-09-11 14:00:00', 'Arsenal', 'Everton', NULL, NULL, NULL),
(1402, '2022-09-11 14:00:00', 'West Ham', 'Newcastle', NULL, NULL, NULL),
(1403, '2022-09-11 16:30:00', 'Crystal Palace', 'Man Utd', NULL, NULL, NULL),
(1404, '2022-09-12 20:00:00', 'Leeds', 'Nottingham Forest', NULL, NULL, NULL),
(1405, '2022-09-16 20:00:00', 'Aston Villa', 'Southampton', NULL, NULL, NULL),
(1406, '2022-09-16 20:00:00', 'Nottingham Forest', 'Fulham', NULL, NULL, NULL),
(1407, '2022-09-17 12:30:00', 'Wolves', 'Man City', NULL, NULL, NULL),
(1408, '2022-09-17 15:00:00', 'Brighton', 'Crystal Palace', NULL, NULL, NULL),
(1409, '2022-09-17 15:00:00', 'Everton', 'West Ham', NULL, NULL, NULL),
(1410, '2022-09-17 15:00:00', 'Newcastle', 'Bournemouth', NULL, NULL, NULL),
(1411, '2022-09-17 17:30:00', 'Spurs', 'Leicester', NULL, NULL, NULL),
(1412, '2022-09-18 14:00:00', 'Brentford', 'Arsenal', NULL, NULL, NULL),
(1413, '2022-09-18 14:00:00', 'Man Utd', 'Leeds', NULL, NULL, NULL),
(1414, '2022-09-18 16:30:00', 'Chelsea', 'Liverpool', NULL, NULL, NULL),
(1415, '2022-10-01 15:00:00', 'Bournemouth', 'Brentford', NULL, NULL, NULL),
(1416, '2022-10-01 15:00:00', 'Arsenal', 'Spurs', NULL, NULL, NULL),
(1417, '2022-10-01 15:00:00', 'Crystal Palace', 'Chelsea', NULL, NULL, NULL),
(1418, '2022-10-01 15:00:00', 'Fulham', 'Newcastle', NULL, NULL, NULL),
(1419, '2022-10-01 15:00:00', 'Leeds', 'Aston Villa', NULL, NULL, NULL),
(1420, '2022-10-01 15:00:00', 'Leicester', 'Nottingham Forest', NULL, NULL, NULL),
(1421, '2022-10-01 15:00:00', 'Liverpool', 'Brighton', NULL, NULL, NULL),
(1422, '2022-10-01 15:00:00', 'Man City', 'Man Utd', NULL, NULL, NULL),
(1423, '2022-10-01 15:00:00', 'Southampton', 'Everton', NULL, NULL, NULL),
(1424, '2022-10-01 15:00:00', 'West Ham', 'Wolves', NULL, NULL, NULL),
(1425, '2022-10-08 15:00:00', 'Bournemouth', 'Leicester', NULL, NULL, NULL),
(1426, '2022-10-08 15:00:00', 'Arsenal', 'Liverpool', NULL, NULL, NULL),
(1427, '2022-10-08 15:00:00', 'Brighton', 'Spurs', NULL, NULL, NULL),
(1428, '2022-10-08 15:00:00', 'Chelsea', 'Wolves', NULL, NULL, NULL),
(1429, '2022-10-08 15:00:00', 'Crystal Palace', 'Leeds', NULL, NULL, NULL),
(1430, '2022-10-08 15:00:00', 'Everton', 'Man Utd', NULL, NULL, NULL),
(1431, '2022-10-08 15:00:00', 'Man City', 'Southampton', NULL, NULL, NULL),
(1432, '2022-10-08 15:00:00', 'Newcastle', 'Brentford', NULL, NULL, NULL),
(1433, '2022-10-08 15:00:00', 'Nottingham Forest', 'Aston Villa', NULL, NULL, NULL),
(1434, '2022-10-08 15:00:00', 'West Ham', 'Fulham', NULL, NULL, NULL),
(1435, '2022-10-15 15:00:00', 'Aston Villa', 'Chelsea', NULL, NULL, NULL),
(1436, '2022-10-15 15:00:00', 'Brentford', 'Brighton', NULL, NULL, NULL),
(1437, '2022-10-15 15:00:00', 'Fulham', 'Bournemouth', NULL, NULL, NULL),
(1438, '2022-10-15 15:00:00', 'Leeds', 'Arsenal', NULL, NULL, NULL),
(1439, '2022-10-15 15:00:00', 'Leicester', 'Crystal Palace', NULL, NULL, NULL),
(1440, '2022-10-15 15:00:00', 'Liverpool', 'Man City', NULL, NULL, NULL),
(1441, '2022-10-15 15:00:00', 'Man Utd', 'Newcastle', NULL, NULL, NULL),
(1442, '2022-10-15 15:00:00', 'Southampton', 'West Ham', NULL, NULL, NULL),
(1443, '2022-10-15 15:00:00', 'Spurs', 'Everton', NULL, NULL, NULL),
(1444, '2022-10-15 15:00:00', 'Wolves', 'Nottingham Forest', NULL, NULL, NULL),
(1445, '2022-10-18 19:45:00', 'Bournemouth', 'Southampton', NULL, NULL, NULL),
(1446, '2022-10-18 19:45:00', 'Arsenal', 'Man City', NULL, NULL, NULL),
(1447, '2022-10-18 19:45:00', 'Brentford', 'Chelsea', NULL, NULL, NULL),
(1448, '2022-10-18 19:45:00', 'Brighton', 'Nottingham Forest', NULL, NULL, NULL),
(1449, '2022-10-18 19:45:00', 'Fulham', 'Aston Villa', NULL, NULL, NULL),
(1450, '2022-10-18 19:45:00', 'Leicester', 'Leeds', NULL, NULL, NULL),
(1451, '2022-10-18 20:00:00', 'Crystal Palace', 'Wolves', NULL, NULL, NULL),
(1452, '2022-10-19 19:45:00', 'Newcastle', 'Everton', NULL, NULL, NULL),
(1453, '2022-10-19 20:00:00', 'Liverpool', 'West Ham', NULL, NULL, NULL),
(1454, '2022-10-19 20:00:00', 'Man Utd', 'Spurs', NULL, NULL, NULL),
(1455, '2022-10-22 15:00:00', 'Aston Villa', 'Brentford', NULL, NULL, NULL),
(1456, '2022-10-22 15:00:00', 'Chelsea', 'Man Utd', NULL, NULL, NULL),
(1457, '2022-10-22 15:00:00', 'Everton', 'Crystal Palace', NULL, NULL, NULL),
(1458, '2022-10-22 15:00:00', 'Leeds', 'Fulham', NULL, NULL, NULL),
(1459, '2022-10-22 15:00:00', 'Man City', 'Brighton', NULL, NULL, NULL),
(1460, '2022-10-22 15:00:00', 'Nottingham Forest', 'Liverpool', NULL, NULL, NULL),
(1461, '2022-10-22 15:00:00', 'Southampton', 'Arsenal', NULL, NULL, NULL),
(1462, '2022-10-22 15:00:00', 'Spurs', 'Newcastle', NULL, NULL, NULL),
(1463, '2022-10-22 15:00:00', 'West Ham', 'Bournemouth', NULL, NULL, NULL),
(1464, '2022-10-22 15:00:00', 'Wolves', 'Leicester', NULL, NULL, NULL),
(1465, '2022-10-29 15:00:00', 'Bournemouth', 'Spurs', NULL, NULL, NULL),
(1466, '2022-10-29 15:00:00', 'Arsenal', 'Nottingham Forest', NULL, NULL, NULL),
(1467, '2022-10-29 15:00:00', 'Brentford', 'Wolves', NULL, NULL, NULL),
(1468, '2022-10-29 15:00:00', 'Brighton', 'Chelsea', NULL, NULL, NULL),
(1469, '2022-10-29 15:00:00', 'Crystal Palace', 'Southampton', NULL, NULL, NULL),
(1470, '2022-10-29 15:00:00', 'Fulham', 'Everton', NULL, NULL, NULL),
(1471, '2022-10-29 15:00:00', 'Leicester', 'Man City', NULL, NULL, NULL),
(1472, '2022-10-29 15:00:00', 'Liverpool', 'Leeds', NULL, NULL, NULL),
(1473, '2022-10-29 15:00:00', 'Man Utd', 'West Ham', NULL, NULL, NULL),
(1474, '2022-10-29 15:00:00', 'Newcastle', 'Aston Villa', NULL, NULL, NULL),
(1475, '2022-11-05 16:00:00', 'Aston Villa', 'Man Utd', NULL, NULL, NULL),
(1476, '2022-11-05 16:00:00', 'Chelsea', 'Arsenal', NULL, NULL, NULL),
(1477, '2022-11-05 16:00:00', 'Everton', 'Leicester', NULL, NULL, NULL),
(1478, '2022-11-05 16:00:00', 'Leeds', 'Bournemouth', NULL, NULL, NULL),
(1479, '2022-11-05 16:00:00', 'Man City', 'Fulham', NULL, NULL, NULL),
(1480, '2022-11-05 16:00:00', 'Nottingham Forest', 'Brentford', NULL, NULL, NULL),
(1481, '2022-11-05 16:00:00', 'Southampton', 'Newcastle', NULL, NULL, NULL),
(1482, '2022-11-05 16:00:00', 'Spurs', 'Liverpool', NULL, NULL, NULL),
(1483, '2022-11-05 16:00:00', 'West Ham', 'Crystal Palace', NULL, NULL, NULL),
(1484, '2022-11-05 16:00:00', 'Wolves', 'Brighton', NULL, NULL, NULL),
(1485, '2022-11-12 16:00:00', 'Bournemouth', 'Everton', NULL, NULL, NULL),
(1486, '2022-11-12 16:00:00', 'Brighton', 'Aston Villa', NULL, NULL, NULL),
(1487, '2022-11-12 16:00:00', 'Fulham', 'Man Utd', NULL, NULL, NULL),
(1488, '2022-11-12 16:00:00', 'Liverpool', 'Southampton', NULL, NULL, NULL),
(1489, '2022-11-12 16:00:00', 'Man City', 'Brentford', NULL, NULL, NULL),
(1490, '2022-11-12 16:00:00', 'Newcastle', 'Chelsea', NULL, NULL, NULL),
(1491, '2022-11-12 16:00:00', 'Nottingham Forest', 'Crystal Palace', NULL, NULL, NULL),
(1492, '2022-11-12 16:00:00', 'Spurs', 'Leeds', NULL, NULL, NULL),
(1493, '2022-11-12 16:00:00', 'West Ham', 'Leicester', NULL, NULL, NULL),
(1494, '2022-11-12 16:00:00', 'Wolves', 'Arsenal', NULL, NULL, NULL),
(1495, '2022-12-26 16:00:00', 'Arsenal', 'West Ham', NULL, NULL, NULL),
(1496, '2022-12-26 16:00:00', 'Aston Villa', 'Liverpool', NULL, NULL, NULL),
(1497, '2022-12-26 16:00:00', 'Brentford', 'Spurs', NULL, NULL, NULL),
(1498, '2022-12-26 16:00:00', 'Chelsea', 'Bournemouth', NULL, NULL, NULL),
(1499, '2022-12-26 16:00:00', 'Crystal Palace', 'Fulham', NULL, NULL, NULL),
(1500, '2022-12-26 16:00:00', 'Everton', 'Wolves', NULL, NULL, NULL),
(1501, '2022-12-26 16:00:00', 'Leeds', 'Man City', NULL, NULL, NULL),
(1502, '2022-12-26 16:00:00', 'Leicester', 'Newcastle', NULL, NULL, NULL),
(1503, '2022-12-26 16:00:00', 'Man Utd', 'Nottingham Forest', NULL, NULL, NULL),
(1504, '2022-12-26 16:00:00', 'Southampton', 'Brighton', NULL, NULL, NULL),
(1505, '2022-12-31 16:00:00', 'Bournemouth', 'Crystal Palace', NULL, NULL, NULL),
(1506, '2022-12-31 16:00:00', 'Brighton', 'Arsenal', NULL, NULL, NULL),
(1507, '2022-12-31 16:00:00', 'Fulham', 'Southampton', NULL, NULL, NULL),
(1508, '2022-12-31 16:00:00', 'Liverpool', 'Leicester', NULL, NULL, NULL),
(1509, '2022-12-31 16:00:00', 'Man City', 'Everton', NULL, NULL, NULL),
(1510, '2022-12-31 16:00:00', 'Newcastle', 'Leeds', NULL, NULL, NULL),
(1511, '2022-12-31 16:00:00', 'Nottingham Forest', 'Chelsea', NULL, NULL, NULL),
(1512, '2022-12-31 16:00:00', 'Spurs', 'Aston Villa', NULL, NULL, NULL),
(1513, '2022-12-31 16:00:00', 'West Ham', 'Brentford', NULL, NULL, NULL),
(1514, '2022-12-31 16:00:00', 'Wolves', 'Man Utd', NULL, NULL, NULL),
(1515, '2023-01-02 16:00:00', 'Arsenal', 'Newcastle', NULL, NULL, NULL),
(1516, '2023-01-02 16:00:00', 'Aston Villa', 'Wolves', NULL, NULL, NULL),
(1517, '2023-01-02 16:00:00', 'Brentford', 'Liverpool', NULL, NULL, NULL),
(1518, '2023-01-02 16:00:00', 'Chelsea', 'Man City', NULL, NULL, NULL),
(1519, '2023-01-02 16:00:00', 'Crystal Palace', 'Spurs', NULL, NULL, NULL),
(1520, '2023-01-02 16:00:00', 'Everton', 'Brighton', NULL, NULL, NULL),
(1521, '2023-01-02 16:00:00', 'Leeds', 'West Ham', NULL, NULL, NULL),
(1522, '2023-01-02 16:00:00', 'Leicester', 'Fulham', NULL, NULL, NULL),
(1523, '2023-01-02 16:00:00', 'Man Utd', 'Bournemouth', NULL, NULL, NULL),
(1524, '2023-01-02 16:00:00', 'Southampton', 'Nottingham Forest', NULL, NULL, NULL),
(1525, '2023-01-14 16:00:00', 'Aston Villa', 'Leeds', NULL, NULL, NULL),
(1526, '2023-01-14 16:00:00', 'Brentford', 'Bournemouth', NULL, NULL, NULL),
(1527, '2023-01-14 16:00:00', 'Brighton', 'Liverpool', NULL, NULL, NULL),
(1528, '2023-01-14 16:00:00', 'Chelsea', 'Crystal Palace', NULL, NULL, NULL),
(1529, '2023-01-14 16:00:00', 'Everton', 'Southampton', NULL, NULL, NULL),
(1530, '2023-01-14 16:00:00', 'Man Utd', 'Man City', NULL, NULL, NULL),
(1531, '2023-01-14 16:00:00', 'Newcastle', 'Fulham', NULL, NULL, NULL),
(1532, '2023-01-14 16:00:00', 'Nottingham Forest', 'Leicester', NULL, NULL, NULL),
(1533, '2023-01-14 16:00:00', 'Spurs', 'Arsenal', NULL, NULL, NULL),
(1534, '2023-01-14 16:00:00', 'Wolves', 'West Ham', NULL, NULL, NULL),
(1535, '2023-01-21 16:00:00', 'Bournemouth', 'Nottingham Forest', NULL, NULL, NULL),
(1536, '2023-01-21 16:00:00', 'Arsenal', 'Man Utd', NULL, NULL, NULL),
(1537, '2023-01-21 16:00:00', 'Crystal Palace', 'Newcastle', NULL, NULL, NULL),
(1538, '2023-01-21 16:00:00', 'Fulham', 'Spurs', NULL, NULL, NULL),
(1539, '2023-01-21 16:00:00', 'Leeds', 'Brentford', NULL, NULL, NULL),
(1540, '2023-01-21 16:00:00', 'Leicester', 'Brighton', NULL, NULL, NULL),
(1541, '2023-01-21 16:00:00', 'Liverpool', 'Chelsea', NULL, NULL, NULL),
(1542, '2023-01-21 16:00:00', 'Man City', 'Wolves', NULL, NULL, NULL),
(1543, '2023-01-21 16:00:00', 'Southampton', 'Aston Villa', NULL, NULL, NULL),
(1544, '2023-01-21 16:00:00', 'West Ham', 'Everton', NULL, NULL, NULL),
(1545, '2023-02-04 16:00:00', 'Aston Villa', 'Leicester', NULL, NULL, NULL),
(1546, '2023-02-04 16:00:00', 'Brentford', 'Southampton', NULL, NULL, NULL),
(1547, '2023-02-04 16:00:00', 'Brighton', 'Bournemouth', NULL, NULL, NULL),
(1548, '2023-02-04 16:00:00', 'Chelsea', 'Fulham', NULL, NULL, NULL),
(1549, '2023-02-04 16:00:00', 'Everton', 'Arsenal', NULL, NULL, NULL),
(1550, '2023-02-04 16:00:00', 'Man Utd', 'Crystal Palace', NULL, NULL, NULL),
(1551, '2023-02-04 16:00:00', 'Newcastle', 'West Ham', NULL, NULL, NULL),
(1552, '2023-02-04 16:00:00', 'Nottingham Forest', 'Leeds', NULL, NULL, NULL),
(1553, '2023-02-04 16:00:00', 'Spurs', 'Man City', NULL, NULL, NULL),
(1554, '2023-02-04 16:00:00', 'Wolves', 'Liverpool', NULL, NULL, NULL),
(1555, '2023-02-11 16:00:00', 'Bournemouth', 'Newcastle', NULL, NULL, NULL),
(1556, '2023-02-11 16:00:00', 'Arsenal', 'Brentford', NULL, NULL, NULL),
(1557, '2023-02-11 16:00:00', 'Crystal Palace', 'Brighton', NULL, NULL, NULL),
(1558, '2023-02-11 16:00:00', 'Fulham', 'Nottingham Forest', NULL, NULL, NULL),
(1559, '2023-02-11 16:00:00', 'Leeds', 'Man Utd', NULL, NULL, NULL),
(1560, '2023-02-11 16:00:00', 'Leicester', 'Spurs', NULL, NULL, NULL),
(1561, '2023-02-11 16:00:00', 'Liverpool', 'Everton', NULL, NULL, NULL),
(1562, '2023-02-11 16:00:00', 'Man City', 'Aston Villa', NULL, NULL, NULL),
(1563, '2023-02-11 16:00:00', 'Southampton', 'Wolves', NULL, NULL, NULL),
(1564, '2023-02-11 16:00:00', 'West Ham', 'Chelsea', NULL, NULL, NULL),
(1565, '2023-02-18 16:00:00', 'Aston Villa', 'Arsenal', NULL, NULL, NULL),
(1566, '2023-02-18 16:00:00', 'Brentford', 'Crystal Palace', NULL, NULL, NULL),
(1567, '2023-02-18 16:00:00', 'Brighton', 'Fulham', NULL, NULL, NULL),
(1568, '2023-02-18 16:00:00', 'Chelsea', 'Southampton', NULL, NULL, NULL),
(1569, '2023-02-18 16:00:00', 'Everton', 'Leeds', NULL, NULL, NULL),
(1570, '2023-02-18 16:00:00', 'Man Utd', 'Leicester', NULL, NULL, NULL),
(1571, '2023-02-18 16:00:00', 'Newcastle', 'Liverpool', NULL, NULL, NULL),
(1572, '2023-02-18 16:00:00', 'Nottingham Forest', 'Man City', NULL, NULL, NULL),
(1573, '2023-02-18 16:00:00', 'Spurs', 'West Ham', NULL, NULL, NULL),
(1574, '2023-02-18 16:00:00', 'Wolves', 'Bournemouth', NULL, NULL, NULL),
(1575, '2023-02-25 16:00:00', 'Bournemouth', 'Man City', NULL, NULL, NULL),
(1576, '2023-02-25 16:00:00', 'Crystal Palace', 'Liverpool', NULL, NULL, NULL),
(1577, '2023-02-25 16:00:00', 'Everton', 'Aston Villa', NULL, NULL, NULL),
(1578, '2023-02-25 16:00:00', 'Fulham', 'Wolves', NULL, NULL, NULL),
(1579, '2023-02-25 16:00:00', 'Leeds', 'Southampton', NULL, NULL, NULL),
(1580, '2023-02-25 16:00:00', 'Leicester', 'Arsenal', NULL, NULL, NULL),
(1581, '2023-02-25 16:00:00', 'Man Utd', 'Brentford', NULL, NULL, NULL),
(1582, '2023-02-25 16:00:00', 'Newcastle', 'Brighton', NULL, NULL, NULL),
(1583, '2023-02-25 16:00:00', 'Spurs', 'Chelsea', NULL, NULL, NULL),
(1584, '2023-02-25 16:00:00', 'West Ham', 'Nottingham Forest', NULL, NULL, NULL),
(1585, '2023-03-04 16:00:00', 'Arsenal', 'Bournemouth', NULL, NULL, NULL),
(1586, '2023-03-04 16:00:00', 'Aston Villa', 'Crystal Palace', NULL, NULL, NULL),
(1587, '2023-03-04 16:00:00', 'Brentford', 'Fulham', NULL, NULL, NULL),
(1588, '2023-03-04 16:00:00', 'Brighton', 'West Ham', NULL, NULL, NULL),
(1589, '2023-03-04 16:00:00', 'Chelsea', 'Leeds', NULL, NULL, NULL),
(1590, '2023-03-04 16:00:00', 'Liverpool', 'Man Utd', NULL, NULL, NULL),
(1591, '2023-03-04 16:00:00', 'Man City', 'Newcastle', NULL, NULL, NULL),
(1592, '2023-03-04 16:00:00', 'Nottingham Forest', 'Everton', NULL, NULL, NULL),
(1593, '2023-03-04 16:00:00', 'Southampton', 'Leicester', NULL, NULL, NULL),
(1594, '2023-03-04 16:00:00', 'Wolves', 'Spurs', NULL, NULL, NULL),
(1595, '2023-03-11 16:00:00', 'Bournemouth', 'Liverpool', NULL, NULL, NULL),
(1596, '2023-03-11 16:00:00', 'Crystal Palace', 'Man City', NULL, NULL, NULL),
(1597, '2023-03-11 16:00:00', 'Everton', 'Brentford', NULL, NULL, NULL),
(1598, '2023-03-11 16:00:00', 'Fulham', 'Arsenal', NULL, NULL, NULL),
(1599, '2023-03-11 16:00:00', 'Leeds', 'Brighton', NULL, NULL, NULL),
(1600, '2023-03-11 16:00:00', 'Leicester', 'Chelsea', NULL, NULL, NULL),
(1601, '2023-03-11 16:00:00', 'Man Utd', 'Southampton', NULL, NULL, NULL),
(1602, '2023-03-11 16:00:00', 'Newcastle', 'Wolves', NULL, NULL, NULL),
(1603, '2023-03-11 16:00:00', 'Spurs', 'Nottingham Forest', NULL, NULL, NULL),
(1604, '2023-03-11 16:00:00', 'West Ham', 'Aston Villa', NULL, NULL, NULL),
(1605, '2023-03-18 16:00:00', 'Arsenal', 'Crystal Palace', NULL, NULL, NULL),
(1606, '2023-03-18 16:00:00', 'Aston Villa', 'Bournemouth', NULL, NULL, NULL),
(1607, '2023-03-18 16:00:00', 'Brentford', 'Leicester', NULL, NULL, NULL),
(1608, '2023-03-18 16:00:00', 'Brighton', 'Man Utd', NULL, NULL, NULL),
(1609, '2023-03-18 16:00:00', 'Chelsea', 'Everton', NULL, NULL, NULL),
(1610, '2023-03-18 16:00:00', 'Liverpool', 'Fulham', NULL, NULL, NULL),
(1611, '2023-03-18 16:00:00', 'Man City', 'West Ham', NULL, NULL, NULL),
(1612, '2023-03-18 16:00:00', 'Nottingham Forest', 'Newcastle', NULL, NULL, NULL),
(1613, '2023-03-18 16:00:00', 'Southampton', 'Spurs', NULL, NULL, NULL),
(1614, '2023-03-18 16:00:00', 'Wolves', 'Leeds', NULL, NULL, NULL),
(1615, '2023-04-01 15:00:00', 'Bournemouth', 'Fulham', NULL, NULL, NULL),
(1616, '2023-04-01 15:00:00', 'Arsenal', 'Leeds', NULL, NULL, NULL),
(1617, '2023-04-01 15:00:00', 'Brighton', 'Brentford', NULL, NULL, NULL),
(1618, '2023-04-01 15:00:00', 'Chelsea', 'Aston Villa', NULL, NULL, NULL),
(1619, '2023-04-01 15:00:00', 'Crystal Palace', 'Leicester', NULL, NULL, NULL),
(1620, '2023-04-01 15:00:00', 'Everton', 'Spurs', NULL, NULL, NULL),
(1621, '2023-04-01 15:00:00', 'Man City', 'Liverpool', NULL, NULL, NULL),
(1622, '2023-04-01 15:00:00', 'Newcastle', 'Man Utd', NULL, NULL, NULL),
(1623, '2023-04-01 15:00:00', 'Nottingham Forest', 'Wolves', NULL, NULL, NULL),
(1624, '2023-04-01 15:00:00', 'West Ham', 'Southampton', NULL, NULL, NULL),
(1625, '2023-04-08 15:00:00', 'Aston Villa', 'Nottingham Forest', NULL, NULL, NULL),
(1626, '2023-04-08 15:00:00', 'Brentford', 'Newcastle', NULL, NULL, NULL),
(1627, '2023-04-08 15:00:00', 'Fulham', 'West Ham', NULL, NULL, NULL),
(1628, '2023-04-08 15:00:00', 'Leeds', 'Crystal Palace', NULL, NULL, NULL),
(1629, '2023-04-08 15:00:00', 'Leicester', 'Bournemouth', NULL, NULL, NULL),
(1630, '2023-04-08 15:00:00', 'Liverpool', 'Arsenal', NULL, NULL, NULL),
(1631, '2023-04-08 15:00:00', 'Man Utd', 'Everton', NULL, NULL, NULL),
(1632, '2023-04-08 15:00:00', 'Southampton', 'Man City', NULL, NULL, NULL),
(1633, '2023-04-08 15:00:00', 'Spurs', 'Brighton', NULL, NULL, NULL),
(1634, '2023-04-08 15:00:00', 'Wolves', 'Chelsea', NULL, NULL, NULL),
(1635, '2023-04-15 15:00:00', 'Aston Villa', 'Newcastle', NULL, NULL, NULL),
(1636, '2023-04-15 15:00:00', 'Chelsea', 'Brighton', NULL, NULL, NULL),
(1637, '2023-04-15 15:00:00', 'Everton', 'Fulham', NULL, NULL, NULL),
(1638, '2023-04-15 15:00:00', 'Leeds', 'Liverpool', NULL, NULL, NULL),
(1639, '2023-04-15 15:00:00', 'Man City', 'Leicester', NULL, NULL, NULL),
(1640, '2023-04-15 15:00:00', 'Nottingham Forest', 'Man Utd', NULL, NULL, NULL),
(1641, '2023-04-15 15:00:00', 'Southampton', 'Crystal Palace', NULL, NULL, NULL),
(1642, '2023-04-15 15:00:00', 'Spurs', 'Bournemouth', NULL, NULL, NULL),
(1643, '2023-04-15 15:00:00', 'West Ham', 'Arsenal', NULL, NULL, NULL),
(1644, '2023-04-15 15:00:00', 'Wolves', 'Brentford', NULL, NULL, NULL),
(1645, '2023-04-22 15:00:00', 'Bournemouth', 'West Ham', NULL, NULL, NULL),
(1646, '2023-04-22 15:00:00', 'Arsenal', 'Southampton', NULL, NULL, NULL),
(1647, '2023-04-22 15:00:00', 'Brentford', 'Aston Villa', NULL, NULL, NULL),
(1648, '2023-04-22 15:00:00', 'Brighton', 'Man City', NULL, NULL, NULL),
(1649, '2023-04-22 15:00:00', 'Crystal Palace', 'Everton', NULL, NULL, NULL),
(1650, '2023-04-22 15:00:00', 'Fulham', 'Leeds', NULL, NULL, NULL),
(1651, '2023-04-22 15:00:00', 'Leicester', 'Wolves', NULL, NULL, NULL),
(1652, '2023-04-22 15:00:00', 'Liverpool', 'Nottingham Forest', NULL, NULL, NULL),
(1653, '2023-04-22 15:00:00', 'Man Utd', 'Chelsea', NULL, NULL, NULL),
(1654, '2023-04-22 15:00:00', 'Newcastle', 'Spurs', NULL, NULL, NULL),
(1655, '2023-04-25 19:45:00', 'Everton', 'Newcastle', NULL, NULL, NULL),
(1656, '2023-04-25 19:45:00', 'Leeds', 'Leicester', NULL, NULL, NULL),
(1657, '2023-04-25 19:45:00', 'Nottingham Forest', 'Brighton', NULL, NULL, NULL),
(1658, '2023-04-25 19:45:00', 'Spurs', 'Man Utd', NULL, NULL, NULL),
(1659, '2023-04-25 19:45:00', 'West Ham', 'Liverpool', NULL, NULL, NULL),
(1660, '2023-04-25 19:45:00', 'Wolves', 'Crystal Palace', NULL, NULL, NULL),
(1661, '2023-04-25 20:00:00', 'Aston Villa', 'Fulham', NULL, NULL, NULL),
(1662, '2023-04-26 19:45:00', 'Chelsea', 'Brentford', NULL, NULL, NULL),
(1663, '2023-04-26 19:45:00', 'Southampton', 'Bournemouth', NULL, NULL, NULL),
(1664, '2023-04-26 20:00:00', 'Man City', 'Arsenal', NULL, NULL, NULL),
(1665, '2023-04-29 15:00:00', 'Bournemouth', 'Leeds', NULL, NULL, NULL),
(1666, '2023-04-29 15:00:00', 'Arsenal', 'Chelsea', NULL, NULL, NULL),
(1667, '2023-04-29 15:00:00', 'Brentford', 'Nottingham Forest', NULL, NULL, NULL),
(1668, '2023-04-29 15:00:00', 'Brighton', 'Wolves', NULL, NULL, NULL),
(1669, '2023-04-29 15:00:00', 'Crystal Palace', 'West Ham', NULL, NULL, NULL),
(1670, '2023-04-29 15:00:00', 'Fulham', 'Man City', NULL, NULL, NULL),
(1671, '2023-04-29 15:00:00', 'Leicester', 'Everton', NULL, NULL, NULL),
(1672, '2023-04-29 15:00:00', 'Liverpool', 'Spurs', NULL, NULL, NULL),
(1673, '2023-04-29 15:00:00', 'Man Utd', 'Aston Villa', NULL, NULL, NULL),
(1674, '2023-04-29 15:00:00', 'Newcastle', 'Southampton', NULL, NULL, NULL),
(1675, '2023-05-06 15:00:00', 'Bournemouth', 'Chelsea', NULL, NULL, NULL),
(1676, '2023-05-06 15:00:00', 'Brighton', 'Everton', NULL, NULL, NULL),
(1677, '2023-05-06 15:00:00', 'Fulham', 'Leicester', NULL, NULL, NULL),
(1678, '2023-05-06 15:00:00', 'Liverpool', 'Brentford', NULL, NULL, NULL),
(1679, '2023-05-06 15:00:00', 'Man City', 'Leeds', NULL, NULL, NULL),
(1680, '2023-05-06 15:00:00', 'Newcastle', 'Arsenal', NULL, NULL, NULL),
(1681, '2023-05-06 15:00:00', 'Nottingham Forest', 'Southampton', NULL, NULL, NULL),
(1682, '2023-05-06 15:00:00', 'Spurs', 'Crystal Palace', NULL, NULL, NULL),
(1683, '2023-05-06 15:00:00', 'West Ham', 'Man Utd', NULL, NULL, NULL),
(1684, '2023-05-06 15:00:00', 'Wolves', 'Aston Villa', NULL, NULL, NULL),
(1685, '2023-05-13 15:00:00', 'Arsenal', 'Brighton', NULL, NULL, NULL),
(1686, '2023-05-13 15:00:00', 'Aston Villa', 'Spurs', NULL, NULL, NULL),
(1687, '2023-05-13 15:00:00', 'Brentford', 'West Ham', NULL, NULL, NULL),
(1688, '2023-05-13 15:00:00', 'Chelsea', 'Nottingham Forest', NULL, NULL, NULL),
(1689, '2023-05-13 15:00:00', 'Crystal Palace', 'Bournemouth', NULL, NULL, NULL),
(1690, '2023-05-13 15:00:00', 'Everton', 'Man City', NULL, NULL, NULL),
(1691, '2023-05-13 15:00:00', 'Leeds', 'Newcastle', NULL, NULL, NULL),
(1692, '2023-05-13 15:00:00', 'Leicester', 'Liverpool', NULL, NULL, NULL),
(1693, '2023-05-13 15:00:00', 'Man Utd', 'Wolves', NULL, NULL, NULL),
(1694, '2023-05-13 15:00:00', 'Southampton', 'Fulham', NULL, NULL, NULL),
(1695, '2023-05-20 15:00:00', 'Bournemouth', 'Man Utd', NULL, NULL, NULL),
(1696, '2023-05-20 15:00:00', 'Brighton', 'Southampton', NULL, NULL, NULL),
(1697, '2023-05-20 15:00:00', 'Fulham', 'Crystal Palace', NULL, NULL, NULL),
(1698, '2023-05-20 15:00:00', 'Liverpool', 'Aston Villa', NULL, NULL, NULL),
(1699, '2023-05-20 15:00:00', 'Man City', 'Chelsea', NULL, NULL, NULL),
(1700, '2023-05-20 15:00:00', 'Newcastle', 'Leicester', NULL, NULL, NULL),
(1701, '2023-05-20 15:00:00', 'Nottingham Forest', 'Arsenal', NULL, NULL, NULL),
(1702, '2023-05-20 15:00:00', 'Spurs', 'Brentford', NULL, NULL, NULL),
(1703, '2023-05-20 15:00:00', 'West Ham', 'Leeds', NULL, NULL, NULL),
(1704, '2023-05-20 15:00:00', 'Wolves', 'Everton', NULL, NULL, NULL),
(1705, '2023-05-28 16:00:00', 'Arsenal', 'Wolves', NULL, NULL, NULL),
(1706, '2023-05-28 16:00:00', 'Aston Villa', 'Brighton', NULL, NULL, NULL),
(1707, '2023-05-28 16:00:00', 'Brentford', 'Man City', NULL, NULL, NULL),
(1708, '2023-05-28 16:00:00', 'Chelsea', 'Newcastle', NULL, NULL, NULL),
(1709, '2023-05-28 16:00:00', 'Crystal Palace', 'Nottingham Forest', NULL, NULL, NULL),
(1710, '2023-05-28 16:00:00', 'Everton', 'Bournemouth', NULL, NULL, NULL),
(1711, '2023-05-28 16:00:00', 'Leeds', 'Spurs', NULL, NULL, NULL),
(1712, '2023-05-28 16:00:00', 'Leicester', 'West Ham', NULL, NULL, NULL),
(1713, '2023-05-28 16:00:00', 'Man Utd', 'Fulham', NULL, NULL, NULL),
(1714, '2023-05-28 16:00:00', 'Southampton', 'Liverpool', NULL, NULL, NULL);
...
(1, '2022-08-05 18:59:00', '2022-08-07 18:35:00'),
(2, '2022-08-13 12:00:00', '2022-08-15 22:00:00'),
(3, '2022-08-20 12:00:00', '2022-08-22 22:00:00'),
(4, '2022-08-27 12:00:00', '2022-08-28 20:00:00');
...
CREATE TABLE `leagues` (
  ...
  `LeagueName` varchar(50) CHARACTER SET latin1 NOT NULL,
  `LeaguePassword` varchar(10) CHARACTER SET latin1 NOT NULL,
  `LeagueDescr` varchar(255) CHARACTER SET latin1 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `passwordresettokens` (
  ...
  `token` varchar(45) CHARACTER SET utf8 NOT NULL,
...
(1, 'resettommy', 'tommygrealy', '2022-08-09 00:00:00'),
(2, '5d6f49c15461a433', ' Matlock', '2022-08-12 22:10:55');
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `playingnotpaid`;
...
CREATE TABLE `predictions` (
  ...
  `PredictionStatus` varchar(8) CHARACTER SET latin1 NOT NULL COMMENT 'A=Active;C=Cancelled',
...
CREATE TABLE `predictionstrash` (
  ...
  `PredictionStatus` varchar(8) CHARACTER SET latin1 NOT NULL COMMENT 'C=Cancelled ; A=Active',
...
('2022-08-02 10:31:13', 'MANUAL', 931, 1, 1337, 'tommygrealy', 'Brentford', 3, 'C', NULL),
('2022-08-02 11:06:48', 'MANUAL', 932, 1, 1335, 'tommygrealy', 'Arsenal', 3, 'C', NULL),
('2022-08-02 11:24:29', 'MANUAL', 933, 1, 1337, 'tommygrealy', 'Leicester', 1, 'C', NULL),
('2022-08-02 11:43:39', 'MANUAL', 934, 1, 1335, 'tommygrealy', 'Crystal Palace', 1, 'C', NULL),
('2022-08-15 22:35:07', 'MANUAL', 952, 3, 1357, 'tommygrealy', 'Everton', 1, 'C', NULL);
...
('2022-08-02 19:19:36', 'MANUAL', 935, 1, 1336, 'tommygrealy', 'Liverpool', 3, 'A', b'0'),
('2022-08-03 08:46:35', 'MANUAL', 936, 1, 1335, 'LordStyxofdeNordsoide', 'Arsenal', 3, 'A', b'1'),
('2022-08-05 16:28:52', 'MANUAL', 937, 1, 1343, 'tommyas', 'Man Utd', 1, 'A', b'0'),
('2022-08-05 17:15:37', 'MANUAL', 938, 1, 1338, 'Bort United', 'Aston Villa', 3, 'A', b'0'),
('2022-08-05 18:45:08', 'MANUAL', 939, 1, 1341, 'deccotter', 'Spurs', 1, 'A', b'1'),
('2022-08-05 18:57:42', 'MANUAL', 940, 1, 1336, 'Patthing', 'Liverpool', 3, 'A', b'0'),
('2022-08-05 19:00:24', 'AUTO', 941, 1, 1335, 'pullingmeplums69', 'Arsenal', 3, 'A', b'1'),
('2022-08-05 19:00:24', 'MANUAL', 942, 1, 1336, 'pguerin', 'Liverpool', 3, 'A', b'0'),
('2022-08-05 19:00:24', 'MANUAL', 943, 1, 1343, 'jamesieoneill@hotmail.com', 'Man Utd', 1, 'A', b'0'),
('2022-08-05 19:00:24', 'AUTO', 944, 1, 1335, 'Jack', 'Arsenal', 3, 'A', b'1'),
('2022-08-05 19:00:24', 'AUTO', 945, 1, 1335, 'Rodgem', 'Arsenal', 3, 'A', b'1'),
('2022-08-10 15:06:57', 'MANUAL', 946, 2, 1351, 'tommygrealy', 'Brentford', 1, 'A', b'1'),
('2022-08-10 17:17:45', 'MANUAL', 947, 2, 1354, 'Bort United', 'Liverpool', 1, 'A', b'0'),
('2022-08-10 17:52:27', 'MANUAL', 948, 2, 1354, 'LordStyxofdeNordsoide', 'Liverpool', 1, 'A', b'0'),
('2022-08-11 22:04:54', 'MANUAL', 949, 2, 1346, 'Krustu', 'Arsenal', 1, 'A', b'1'),
('2022-08-12 11:25:13', 'MANUAL', 950, 2, 1348, 'Mheg2021', 'Man City', 1, 'A', b'1'),
('2022-08-12 16:23:21', 'MANUAL', 951, 2, 1348, 'Patthing', 'Man City', 1, 'A', b'1'),
('2022-08-19 12:58:57', 'MANUAL', 953, 3, 1361, 'Mheg2021', 'Chelsea', 3, 'A', b'0'),
('2022-08-19 13:01:42', 'MANUAL', 954, 3, 1360, 'Krustu', 'Bournemouth', 1, 'A', b'0'),
('2022-08-20 11:01:26', 'MANUAL', 955, 3, 1355, 'Patthing', 'Spurs', 1, 'A', b'1'),
('2022-08-20 11:06:38', 'MANUAL', 956, 3, 1357, 'tommygrealy', 'Everton', 1, 'A', b'0');
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `thisweeksfixtures`;
...
CREATE TABLE `userfeedback` (
  ...
  `commentUserName` varchar(100) CHARACTER SET utf8 DEFAULT NULL,
  `commentEmail` varchar(100) CHARACTER SET utf8 DEFAULT NULL,
...
INSERT INTO `users` (`id`, `username`, `FullName`, `password`, `salt`, `PrivLevel`, `email`, `lives`, `CompStatus`, `PaymentStatus`) VALUES
(2076, 'Patthing', 'Pat Collins ', 'f4a32980e006988cb683d4910dcd94948b18de4623dcc3550a47e3021bbbd95f', '180bbcb942553031', 1, 'patthing@gmail.com', 3, 'Playing', 'Pending'),
(2077, 'Matlock', 'Matlock QC', '0bccc9fe8d210f52eceb4ed275bb20bf923ac552289b07ef17b0622ac2c320a7', '5ee0b22c49af6856', 1, 'bclarkie@gmail.com', 3, 'Playing', 'Pending'),
(2078, 'pullingmeplums69', 'Philip Flynn', '0eef60740951773d24b9eb379267a9a60a397719906febf714634a8bf62d8ff8', '61eaad8d5aec04af', 1, 'philipflynn76@gmail.com', 3, 'Playing', 'Pending'),
(2079, 'LordStyxofdeNordsoide', 'Styx', '01cabca0e910767248f51924844cc023b843e90d7c45fc7d9b447b8bf2607984', 'a60428c71df0551', 1, 'd_oshaughnessy@yahoo.com', 3, 'Playing', 'Pending'),
(2080, 'Mheg2021', 'Cairbre O Donnell', 'd9466f0a4b6178f21ab84dc2ca962683af3b4050a6eaf5d23bc3396b21e9d435', '6e5d6aa6ffeb1b8', 1, 'cairbre.odonnell@me.com', 3, 'Playing', 'Pending'),
(2081, 'Bort United', 'Thomas Bartlett', '524146a99fcd2dacdc31f3677cc679e141297df6c551cbd9c1d0ced3416a398c', '710ee7e579ea7f6e', 1, 'thomasbartlet@gmail.com', 3, 'Playing', 'Pending'),
(2082, 'Krustu', 'Krustu', 'a6499235ef6935327ef664ed89288e4507f55f63eeb4951d24f50685f202d62d', '284d0a0231278c06', 1, 'b_o_farrell@yahoo.com', 3, 'Playing', 'Pending'),
(2086, 'Hutto', 'Dougie Hutton', 'cf3a639345b71cd05c8a08ca4931a6eb88f84dae64ce8ed634f8348f8c14a019', '71f4950a4717afdf', 1, 'huttod76@gmail.com', 3, 'Playing', 'Pending'),
(2088, 'tommygrealy', 'Tommy Grealy', 'e42828e1f1f4aaf292f0c05ebda016a514c1a34ba0489010e318e56ebaa93420', '133755b24e09f584', 3, 'tommygrealy@gmail.com', 3, 'Playing', 'Pending'),
(2098, 'tommyas', 'Tommy Grealy', '81e88d0676136814734d5cee7070b4b6bb63d06e6aab2793efcda690f8471775', '2c5f48af1d3c3a0e', 3, 'tommy@actionshots.ie', 3, 'Playing', 'Pending'),
(2099, 'pguerin', 'Pat G', '7c262671cff032a7823be3579bbe17f5077afefa398e2022b66c195dae55bbb1', '8637a6d9b445a2', 1, 'guerinp@gmail.com', 3, 'Playing', 'Pending'),
(2103, 'jamesieoneill@hotmail.com', 'james o neill', 'f2ba59e71628c3dbec151f171f90bedc1bdd7c0cfb1660ccf02c94dfbd067b23', 'c98c66f4dbe87d5', 1, 'jamesieoneill@hotmail.com', 3, 'Playing', 'Pending'),
(2104, 'Jack', 'John Bligh', 'c1d580c63d73c60e80139446b8f15e327ca98c6b13a0ad58c22df25e05718e12', '513fe127690f42f', 1, 'Johnbligh@gmail.com', 3, 'Playing', 'Pending'),
(2105, 'Rodgem', 'Rodger mooney', 'df11374edc299d61cd3339411a2c1420444c301978fbad2089bbacac351905b2', '71f943b33d620f0', 1, 'Rodgemooney@gmail.com', 3, 'Playing', 'Pending'),
(2106, 'deccotter', 'Declan Cotter', 'a2e1cc7e1dc70c9e2a3f82bd2a8636acf445c2e6c44b4f91ea080fc2db98e2a8', '3a55ba6b51ae7981', 1, 'Deccotter76@gmail.com', 3, 'Playing', 'Pending');
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `usersnotsubmitted`;
...
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `users_join_leagues`;
...
--
ALTER TABLE `league_memberships`
  MODIFY `league_mem_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=30;

--
-- AUTO_INCREMENT for table `passwordresettokens`
--
ALTER TABLE `passwordresettokens`
  MODIFY `idpasswordResetTokens` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `predictions`
--
ALTER TABLE `predictions`
  MODIFY `PredictionID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=957;

--
-- AUTO_INCREMENT for table `predictionstrash`
--
ALTER TABLE `predictionstrash`
  MODIFY `PredictionID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=953;
