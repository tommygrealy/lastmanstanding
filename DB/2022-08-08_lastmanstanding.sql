...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `cancelPrediction` (IN `inPredictionID` INT(11), IN `inUserName` VARCHAR(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkResultsVsPredictions` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `generateResetForUsername` (IN `inUsername` VARCHAR(255), IN `inToken` VARCHAR(45))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getNextFixtureForTeam` (IN `TeamNameLong` VARCHAR(50))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUserDetailsFromToken` (IN `inToken` VARCHAR(45))  NO SQL
select username from users where username = 
(select username from passwordresettokens where token = inToken and expiry > (select NOW()))$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `insertPrediction` (IN `inFixtureID` INT(11), IN `inUserName` VARCHAR(255), IN `inPredictedResult` INT(11), IN `inEntryType` VARCHAR(10))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `passwordReset` (IN `inUsername` VARCHAR(255), IN `inPassword` VARCHAR(64), IN `inSalt` VARCHAR(16))  NO SQL
update users 
set password=inPassword, salt=inSalt
where username = inUsername$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `selectRandomTeam` (IN `inUser` VARCHAR(255))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showAvailableTeamsForUser` (IN `inUserName` VARCHAR(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showCurrentSelections` ()  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showCurrentStandings` ()  BEGIN
select username, FullName, CompStatus from users where PaymentStatus='Paid';

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `showCurrentStandingsByLeague` (IN `league_id` INT(11))  BEGIN
select username, FullName, CompStatus from users where PaymentStatus='Paid' 
AND username IN (SELECT username from users_join_leagues WHERE users_join_leagues.league_id = league_id);

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `showNullResultFixtures` ()  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showPredsByGameWeek` (IN `inGameWeek` INT)  NO SQL
select * from predictions where gameweek=inGameWeek$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `showSelectionsPostDeadline` ()  NO SQL
    COMMENT 'Shows all user selections for the next round of games.'
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showUserCurrentSelection` (IN `inUserName` VARCHAR(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `showUserPredictionHistory` (IN `inUsername` VARCHAR(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateMatchScore` (IN `inFixtureID` INT(11), IN `inHomeScore` INT(11), IN `inAwayScore` INT(11), IN `inResult` INT(11))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updatePaymentStatus` (IN `inUserName` VARCHAR(255), IN `inPayStat` VARCHAR(45))  BEGIN
...
(2, '2022-08-13 12:30:00', '2022-08-15 22:00:00'),
(3, '2022-08-20 12:30:00', '2022-08-22 22:00:00');
...
(2076, 'Patthing', 'Pat Collins ', 'f4a32980e006988cb683d4910dcd94948b18de4623dcc3550a47e3021bbbd95f', '180bbcb942553031', 1, 'patthing@gmail.com', 'Playing', 'Pending'),
(2077, 'Matlock', 'Matlock QC', '0bccc9fe8d210f52eceb4ed275bb20bf923ac552289b07ef17b0622ac2c320a7', '5ee0b22c49af6856', 1, 'bclarkie@gmail.com', 'Playing', 'Pending'),
(2078, 'pullingmeplums69', 'Philip Flynn', '0eef60740951773d24b9eb379267a9a60a397719906febf714634a8bf62d8ff8', '61eaad8d5aec04af', 1, 'philipflynn76@gmail.com', 'Playing', 'Pending'),
(2079, 'LordStyxofdeNordsoide', 'Styx', '01cabca0e910767248f51924844cc023b843e90d7c45fc7d9b447b8bf2607984', 'a60428c71df0551', 1, 'd_oshaughnessy@yahoo.com', 'Playing', 'Pending'),
(2080, 'Mheg2021', 'Cairbre O Donnell', 'd9466f0a4b6178f21ab84dc2ca962683af3b4050a6eaf5d23bc3396b21e9d435', '6e5d6aa6ffeb1b8', 1, 'cairbre.odonnell@me.com', 'Playing', 'Pending'),
(2081, 'Bort United', 'Thomas Bartlett', '524146a99fcd2dacdc31f3677cc679e141297df6c551cbd9c1d0ced3416a398c', '710ee7e579ea7f6e', 1, 'thomasbartlet@gmail.com', 'Playing', 'Pending'),
(2082, 'Krustu', 'Krustu', 'a6499235ef6935327ef664ed89288e4507f55f63eeb4951d24f50685f202d62d', '284d0a0231278c06', 1, 'b_o_farrell@yahoo.com', 'Playing', 'Pending'),
(2086, 'Hutto', 'Dougie Hutton', 'cf3a639345b71cd05c8a08ca4931a6eb88f84dae64ce8ed634f8348f8c14a019', '71f4950a4717afdf', 1, 'huttod76@gmail.com', 'Playing', 'Pending'),
(2088, 'tommygrealy', 'Tommy Grealy', 'e42828e1f1f4aaf292f0c05ebda016a514c1a34ba0489010e318e56ebaa93420', '133755b24e09f584', 3, 'tommygrealy@gmail.com', 'Playing', 'Pending'),
(2098, 'tommyas', 'Tommy Grealy', '81e88d0676136814734d5cee7070b4b6bb63d06e6aab2793efcda690f8471775', '2c5f48af1d3c3a0e', 1, 'tommy@actionshots.ie', 'Playing', 'Pending'),
(2099, 'pguerin', 'Pat G', '7c262671cff032a7823be3579bbe17f5077afefa398e2022b66c195dae55bbb1', '8637a6d9b445a2', 1, 'guerinp@gmail.com', 'Playing', 'Pending'),
(2103, 'jamesieoneill@hotmail.com', 'james o neill', 'f2ba59e71628c3dbec151f171f90bedc1bdd7c0cfb1660ccf02c94dfbd067b23', 'c98c66f4dbe87d5', 1, 'jamesieoneill@hotmail.com', 'Playing', 'Pending'),
(2104, 'Jack', 'John Bligh', 'c1d580c63d73c60e80139446b8f15e327ca98c6b13a0ad58c22df25e05718e12', '513fe127690f42f', 1, 'Johnbligh@gmail.com', 'Playing', 'Pending'),
(2105, 'Rodgem', 'Rodger mooney', 'df11374edc299d61cd3339411a2c1420444c301978fbad2089bbacac351905b2', '71f943b33d620f0', 1, 'Rodgemooney@gmail.com', 'Playing', 'Pending'),
(2106, 'deccotter', 'Declan Cotter', 'a2e1cc7e1dc70c9e2a3f82bd2a8636acf445c2e6c44b4f91ea080fc2db98e2a8', '3a55ba6b51ae7981', 1, 'Deccotter76@gmail.com', 'Playing', 'Pending');
...
--
ALTER TABLE `passwordresettokens`
  MODIFY `idpasswordResetTokens` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
-- AUTO_INCREMENT for table `predictions`
--
ALTER TABLE `predictions`
  MODIFY `PredictionID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=946;
--
-- AUTO_INCREMENT for table `predictionstrash`
--
ALTER TABLE `predictionstrash`
  MODIFY `PredictionID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=935;
--
